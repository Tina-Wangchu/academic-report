...
> - **后续 ✅ 配置目录 config/→assets/ 重命名（2026-08-11）**：用户将运行期数据/配置目录 `academic-report/config/` 重命名为 `assets/`（更准确表达「配置 + 运行期数据」）。`utils.get_skill_data_dir()` 返回值 `config`→`assets`（**功能性同步——否则程序读不到 assets/.env**）；`utils`/`config_manager`/`pipeline`/`email_sender` 内 docstring 与注释、`README.md` 目录树（同步移除已删除的 `templates/`、补上 `reports/`）与正文路径、`.gitignore` 注释一并改为 `assets/`。`.gitignore` 规则均为通配（`*.env`/`email_sends.jsonl`/`llm_cache_*.json`/`academic_scholar_timestamps.json`），对新路径自动生效，无需改规则。
>
