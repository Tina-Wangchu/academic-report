...
DEFAULT_LOG_PATH = Path.home() / ".hermes" / "email_sends.jsonl"
...
DEFAULT_COOLDOWN_PATH = Path.home() / ".hermes" / "email_send_cooldown.json"
...
class EmailSender:
    ...
    def __init__(self, config_manager=None, max_retries: int = 3,
        ...
        Args:
            ...
            retry_delay:    重试间隔秒数
            timeout:        SMTP 连接超时
            log_path:       发送记录日志路径（默认 ~/.hermes/email_sends.jsonl）
            cooldown_path:  冷却守卫状态路径（默认 ~/.hermes/email_send_cooldown.json）
        """
